...
class TechnoParkScaper:
    ...
    def scrape_job_details(self, post_result_set: ResultSet) -> list:
        title = post_result_set.td.text
        url_path = post_result_set.td.a.get("href")
        cname = post_result_set.select_one("td:nth-of-type(2)").text
        closing_date = post_result_set.select_one("td:nth-of-type(3)").text
        if not url_path:
            return
    ...
    def scrape(self):
        with open('data.csv', 'w', encoding="UTF-8") as f:
            ...
            for job_post in total_job_post_list:
                job_details = self.scrape_job_details(job_post)
